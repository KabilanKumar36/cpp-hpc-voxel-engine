...
public:
	...
	bool CheckCollision(const AABB& Other) const {
		if (m_objMaxPt.x < Other.m_objMinPt.x || m_objMinPt.x > Other.m_objMaxPt.x) return false;
		if (m_objMaxPt.y < Other.m_objMinPt.y || m_objMinPt.y > Other.m_objMaxPt.y) return false;
		if (m_objMaxPt.z < Other.m_objMinPt.z || m_objMinPt.z > Other.m_objMaxPt.z) return false;
		return true;
	...
	Core::Vec3 GetCenter() const {
		return (m_objMinPt + m_objMaxPt) * 0.5f;
	}
};