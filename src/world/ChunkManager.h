    ...
    void Update(float fPlayerX, float fPlayerZ) { 
        ...
        while ((optChunk = m_objFinishedQueue.try_pop()).has_value()) {
            ...
            m_mapChunks.insert(std::make_pair(
                std::make_pair(objChunk.GetChunkX(), objChunk.GetChunkZ()), 
                std::move(objChunk))
            );
        }
        ...
        for(int iX = iCurrentChunkX - m_iRenderDistance; iX <= iCurrentChunkX + m_iRenderDistance; iX++){
            for(int iZ = iCurrentChunkZ - m_iRenderDistance; iZ <= iCurrentChunkZ + m_iRenderDistance; iZ++){
                std::pair<int, int> ChunkCoord = {iX, iZ};
                if(m_mapChunks.count(ChunkCoord)) continue;

                bool bPending = false;
                {
                    std::lock_guard<std::mutex> lock(m_mutexPending);
                    if(m_setPendingCoords.count(ChunkCoord)) bPending = true;
                }
