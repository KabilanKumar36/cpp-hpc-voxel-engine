...
void InputHandler::MouseCallback(GLFWwindow* pWindow, double xPosIn, double yPosIn) {
    ...
    void InputHandler::ScrollCallback([[maybe_unused]]GLFWwindow* pWindow, [[maybe_unused]]double dXOffset, double dYOffset) {
        if(m_bPerspective)
	...
	void InputHandler::processFirePreviewAndFire(GLFWwindow *pWindow, std::vector<Chunk> &chunks, 
		const Core::Mat4 & viewProjection)
	{
		...
		if (glfwGetKey(pWindow, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS || glfwGetKey(pWindow, GLFW_KEY_RIGHT_CONTROL) == GLFW_PRESS)
		{
			...
			Core::Ray objRay(App::InputHandler::GetCamera().GetCameraPosition(), App::InputHandler::GetCamera().GetFront());
			...
			Renderer::PrimitiveRenderer::DrawLine(objRayStart, objRayEnd, Core::Vec3(1.0f, 1.0f, 0.0f), 
			viewProjection);
	...
	#endif
				Renderer::PrimitiveRenderer::DrawCube(objBlockPos, Core::Vec3(1.005f, 1.005f, 1.005f), 
    ...
    void InputHandler::FramebufferSizeCallback([[maybe_unused]]GLFWwindow* window, int width, int height) {
	...
	Core::Mat4 InputHandler::GetViewProjectionMatrix() {
		...
		{
			projection = Core::Mat4::perspective(m_objCamera.GetZoom(),
